...
const hexToBase64 = (h) => {
  return btoa(h.replace(/0x/gi,'').match(/\w{2}/g).map(function(a) {
...
let self = module.exports = {
  ...
  checkStateAndConnect: ( manager ) => {
    return new Promise ( (resolve, reject) => {
      ...
      const subscription = manager.onStateChange( (stateOfDev) => {
        console.log('DEBUG - Bluetooth.js checkStateAndConnect() stateOfDev ', stateOfDev)
  ...
  scanAndConnect: ( manager, dev )  => {
    return new Promise ( (resolve, reject) => {
      manager.startDeviceScan(null, null, (error, bleDev) => {
        ...
        console.log('DEBUG - Bluetooth.js scanAndConnect() ', bleDev.id, dev.id)
        ...
        if (bleDev.id === dev.id) {    
          ...
          self.connectToBleDevice( bleDev )
            .then (device => resolve(device) )
            .catch( (err) => reject('ERROR - could not connect ', err) )
        }
      })
  ...
  connectToBleDevice: (bleDev) => {
    return new Promise ( (resolve, reject) => {
          bleDev.connect()
            ...
            .then( (device) => {
              if (device.name === null || device.name.length < 2) device.name = device.id
              if (device.localName === null  || device.name.length < 2) device.localName = device.name
              resolve( device )
            })
            .catch( (err) => reject('ERROR - could not connect ', err) )
    })
  ...
  scanForDevices: ( manager, addDevice ) => {
      manager.startDeviceScan(null, null, (error, bleDev) => {
        ...
        if (bleDev.name === null || bleDev.name.length < 2) bleDev.name = bleDev.id
        if (bleDev.localName === null  || bleDev.name.length < 2) bleDev.localName = bleDev.name
        addDevice( bleDev )
      })
  ...
  sendOnToBluetooth: (manager, devId, channel) => {
    ...
    console.log('DEBUG - test ', devId, bV[channel], hexToBase64('0xA00101A2'), hexToBase64('0xA00101A3'))
    if (DEBUGMODE) return('Debug')
    manager.writeCharacteristicWithoutResponseForDevice( devId, SUUID, CUUID, bV[channel] )
      .then( (res) => {
  ...
  disconnectFromBleDevice: (manager, devId) => {
    return new Promise ( (resolve, reject) => {
        manager.cancelDeviceConnection(devId)
        .then( (res) => {
          resolve(res)
        })
        .catch( (err) => {
